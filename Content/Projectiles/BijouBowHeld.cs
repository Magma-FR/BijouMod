...
{
    public class BijouBowHeld : ModProjectile
    {
        private float AimResponsiveness = 0.69f;
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] >= 90f)
            {
                ...
                for (int i = 0; i < 50; i++)
                {
                    ...
                    Dust dust1 = Dust.NewDustPerfect(Projectile.Center, DustID.BlueCrystalShard, speed * 7, Scale: 1.2f);
        ...
        {
            Player player = Main.player[Projectile.owner];
            SoundEngine.PlaySound(Soundd.BowShot, Projectile.position);

            if (Main.myPlayer == Projectile.owner)
            {
                float numberProjectiles = 3 + Main.rand.Next(0, 0);
                ...
                for (int i = 0; i < numberProjectiles; i++)
                {
                    ...
                    Projectile.NewProjectile(player.GetSource_ItemUse_WithPotentialAmmo(player.HeldItem, AmmoID.Bullet), Projectile.position, perturbedSpeed, ModContent.ProjectileType<RizzArrow>(), Projectile.damage, Projectile.knockBack, player.whoAmI);
                }
            }
